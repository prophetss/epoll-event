...
{
    int ret;
    *ep_element = malloc(sizeof(ep_element_t));
    if (!(*ep_element)) {
        _exit_throw("failed to memory allocation!\n");
    }
    if ((ret = hash_table_add(ep_event->table, &fd, sizeof(int), *ep_element, sizeof(ep_element_t))) < 0) {
        _error_display("failed to add element for hashtable, sock id: %d\n", fd);
        return -2;
    }
    (*ep_element)->fd = fd;
...
static int ep_event_process(ep_event_t *ep_event)
{
    ...
    /*错误*/
    if (-1 == fds) {
        _exit_throw("failed to epoll_wait!\n");
    }
    /*超时*/
    ...
    for (int i = 0; i < fds; i++) {
        ...
        if (NULL == (element = (ep_element_t*)hash_table_lookup(ep_event->table, &events[i].data.fd, sizeof(int)))) {
            /*没找到，异常*/
            _error_display("not found hashtable for sock id: %d\n", events[i].data.fd);
            continue;
        }
